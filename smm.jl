...
function multiple_starts(n::Int64, sc_active::Bool, home_prod::Bool, max_iter::Int64=200)
    ...
    for i in 1:n
        ...
        opt = optimize(optim_f, init, NelderMead(), Optim.Options(show_trace=true, show_every=1, iterations=max_iter))
        Min_y[i] = opt.minimum
...
function optimum_w_optimal_weights(n::Int64, sc_active::Bool, home_prod::Bool, max_iter::Int64=200)
    ...
    opt = optimize(optim_f, vec(x_min), NelderMead(), Optim.Options(show_trace=true, show_every=1, iterations=max_iter))
